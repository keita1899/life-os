...
function categorizeEvent(
  ...
  if (eventStartDate < today) return 'overdue'
  return 'future'
}

function createDateGroup(dateStr: string): EventGroup {
  return { key: dateStr, title: formatDateDisplay(dateStr), events: [] }
}
...
export function groupEvents(events: Event[]): EventGroup[] {
  ...
  events.forEach((event) => {
    ...
    switch (category) {
      ...
      case 'future': {
        const eventDateStr = getEventDateString(event)
        if (!dateGroups.has(eventDateStr)) dateGroups.set(eventDateStr, [])
