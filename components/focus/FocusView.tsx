...
interface FocusViewProps<T extends TaskLike> {
  tasks: T[]
  isLoading: boolean
  error: string | null
  onCompleteTask: (taskId: number, timeMinutes?: number) => Promise<void>
  renderAvailableItem: (task: T, onToggle: () => void) => React.ReactNode
  renderFocusItem: (
    task: T,
    index: number,
    onToggle: () => void,
...
export function FocusView<T extends TaskLike>({
  tasks,
  isLoading,
  error,
  onCompleteTask,
  ...
  const {
    ...
    onCompleteTask: async (taskId, timeMinutes) => {
      await onCompleteTask(taskId, timeMinutes)
    },
  })
                        ...
                        {availableTasks.map((task: T) => {
                          ...
                          return (
                            <div key={task.id}>
                              ...
                              {renderAvailableItem(task, () => toggleTask(task.id))}
                            </div>
                          )
                        })}
                        {activeId !== null &&
                ...
                <div className="space-y-4">
                  ...
                  {focusTasks.length === 0 ? (
                    ...
                    >
                      ...
                      >
                        <div className="space-y-2">
                          {focusTasks.map((task: T, index: number) => {
                            const showSpacerAbove =
                              activeId !== null &&
                              !focusTaskIds.includes(activeId) &&
                              overId === task.id
